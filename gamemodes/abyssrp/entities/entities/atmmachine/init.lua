...
concommand.Add( "bank_deposit", function( ply, cmd, args )
	...
	ply:TakeCash(amount)
	ply:AddBank(amount)
	RP:Notify( ply, RP.colors.white, "You deposited ", RP.colors.blue, "$"..amount, RP.colors.white, " to your bank account.")
...
concommand.Add( "bank_withdraw", function( ply, cmd, args )
	...
	ply:AddCash(amount)
	ply:TakeBank(amount)
	RP:Notify( ply, RP.colors.white, "You withdrew ", RP.colors.blue, "$"..amount, RP.colors.white, " from your bank account.")
	umsg.Start("ATMUpdate", ply)
		umsg.Float(ply:GetBank())
	umsg.End()
end )
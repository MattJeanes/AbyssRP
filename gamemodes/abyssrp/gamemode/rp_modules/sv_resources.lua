/*-------------------------------------------------------------------------------------------------------------------------
	Send some files to the client
-------------------------------------------------------------------------------------------------------------------------*/

resource.AddFile("materials/vgui/vehicles/jeep.vmt")
resource.AddFile("materials/vgui/vehicles/airboat.vmt")

resource.AddFile("models/env/misc/bank_atm/bank_atm.mdl")
resource.AddFile("materials/ENV/OBJ/bank/bank_atm.vmt")

resource.AddFile("sound/player/slide/slide_floor_1.wav")
resource.AddFile("sound/player/slide/slide_floor_2.wav")
resource.AddFile("sound/player/slide/slide_floor_3.wav")
resource.AddFile("sound/player/slide/slide_loop_me.wav")
resource.AddFile("sound/player/slide/slide_loop4.wav")
resource.AddFile("sound/player/slide/slide_start_1.wav")
...
concommand.Add( "bank_deposit", function( ply, cmd, args )
	...
	if (not amount) or (amount < 1) then return end
		if tonumber(ply:GetNWInt("cash")) >= amount then
			ply:SetNWInt("cash", ply:GetNWInt( "cash" ) - amount);
			ply:SetNWInt("bank", ply:GetNWInt( "bank" ) + amount);
			ply:SavePlayerData()
...
concommand.Add( "bank_withdraw", function( ply, cmd, args )
	local amount = tonumber(args[1])
	if (not amount) or (amount < 1) then return end
		if tonumber(ply:GetNWInt("bank")) >= amount then
			ply:SetNWInt("cash", ply:GetNWInt( "cash" ) + amount);
			ply:SetNWInt("bank", ply:GetNWInt("bank") - amount)
			ply:SavePlayerData()

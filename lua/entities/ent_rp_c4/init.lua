...
function ENT:Think()
	...
	if self.Defuse >= 1 then
		local Success = tobool(math.Round(math.Rand(0,1)))
		if Success then
			self.Entity:Remove()
			self:EmitSound("C4.DisarmFinish")
			self.Defuser:PrintMessage(HUD_PRINTTALK, "You've defused the bomb!")
		else
			self.Defuser:PrintMessage(HUD_PRINTTALK, "You've failed to defuse the bomb!")
			self:Explosion() 
		end
	end
...
function ENT:Explosion()
	...
	util.Effect("effect_rp_ignition", effectdata)

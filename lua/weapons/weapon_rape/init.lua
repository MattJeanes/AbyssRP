...
concommand.Add( SWEP.PRIMARYPW, function( ply, cmd, args )
	...
	if not ply:HasWeapon( "weapon_rape" ) then return end --idc if ppl exploit the command
	InProgress = true
	...
	local ragVictim = ents.Create("prop_ragdoll")
	ragVictim:SetModel( plyVictim:GetModel() )
	ragVictim:SetPos( plyVictimPos )
	ragVictim:Spawn()
	ragVictim:Activate()
	
	for i = 1, ragVictim:GetPhysicsObjectCount() do
		local phys = ragVictim:GetPhysicsObjectNum(i)
		if phys and phys:IsValid() then
			phys:EnableCollisions( true )
			phys:EnableMotion( false )
			phys:SetPos( (ragVictim:GetPos()-Vector(0,0,40)) + victimPos[i] )
	...
	for i = 1, ragAttacker:GetPhysicsObjectCount() do
		local phys = ragAttacker:GetPhysicsObjectNum(i)
		if phys and phys:IsValid() then
			phys:SetPos( (ragVictim:GetPos()-Vector(0,0,40)) + attackerPos[i] )
	...
	local emitSoundPos = ragVictim:GetPos()
	timer.Create( soundTimerString, SoundDelay, 0, function()
		sound.Play( sounds[math.random(#sounds)], emitSoundPos, 100, 150 )
	end )
	...
	timer.Simple( RapeLength, function()
		...
		if plyVictim and plyVictim:IsValid() then
			...
			plyVictim:SetHealth( plyVictim:Health() / 2 )
		end
		
		if ragAttacker and ragAttacker:IsValid() then ragAttacker:Remove() end
		if ragVictim and ragVictim:IsValid() then ragVictim:Remove() end

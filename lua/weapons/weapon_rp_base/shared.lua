...
function SWEP:ShootEffects()
	...
	timer.Simple(0, function()
		...
		if (self.Shotgun) then
			util.Effect("effect_rp_shotgunsmoke", effectdata)
		else
			util.Effect("effect_rp_gunsmoke", effectdata)
		end
	end)
...
function SWEP:ShootFire(attacker, tr, dmginfo)
	...
	util.Effect("effect_rp_firehit", effectdata)
...
function SWEP:ShootAnimation()
	...
	local AllowDryFire = self.Owner:GetActiveWeapon():GetClass() == ("weapon_rp_deagle") 
				   or self.Owner:GetActiveWeapon():GetClass() == ("weapon_rp_usp") 
				   or self.Owner:GetActiveWeapon():GetClass() == ("weapon_rp_usp_match")
...
function SWEP:ShootBullet(damage, recoil, num_bullets, aimcone)
	...
	if (SERVER and (self.Sniper or self.Shotgun) and not self.Owner:GetActiveWeapon():GetClass() == ("weapon_rp_admin")) then
...
function SWEP:BulletPenetrate(bouncenum, attacker, tr, dmginfo, isplayer)
	...
	if self.Primary.Ammo == "5.56MM" then // 5.56MM Ammo
		MaxPenetration = 18
	elseif self.Primary.Ammo == "4.6MM" then // 4.6MM Ammo
		MaxPenetration = 8
	elseif self.Primary.Ammo == "5.7MM" then // 5.7MM Ammo
		MaxPenetration = 12
	elseif self.Primary.Ammo == "9MM" then // 9MM Ammo
		MaxPenetration = 14
	elseif self.Primary.Ammo == "7.62MM" then // 7.62MM Ammo
		MaxPenetration = 20
	elseif self.Primary.Ammo == ".45MM" then // .45 Ammo
		MaxPenetration = 16
	elseif self.Primary.Ammo == ".50MM" then // .50 Ammo
		MaxPenetration = 20
	else
		MaxPenetration = 16
	end
	...
	{	
		...
		Spread 	= Vector(0, 0, 0),
		Tracer	= 1,
		TracerName 	= "effect_rp_penetration_trace",
		Force		= 5,
...
function SWEP:RicochetCallback(bouncenum, attacker, tr, dmginfo)
	...
	{	
		...
		Spread 	= Vector(0, 0, 0),
		Tracer	= 1,
		TracerName 	= "effect_rp_ricochet_trace",
